...
import { useCart } from './useCart.js';  //5-1. 커스텀훅끼리 연동가능함 
...
export function useOrder() {
    ...
    const {calculateTotalPrice} = useCart();  //5-1.
    ...
    const getOrderList = async() => {
        ...
        setOrderList(result.data);  //3.orderlist 에 db 에서 받아온값이 들어잇음 
        setMember(result.data[0]); //4-1.
        calculateTotalPrice(result.data); //5-1.
