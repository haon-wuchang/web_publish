...
--      단, 고객아이디를 기준으로 오름차순 정렬해서 조회하세요.
        ...
        desc order_header;
        select * from order_header 
			where order_date between '2019-01-01'and '2019-01-08' order by customer_id;
    
-- Q29) 2019-01-01 ~ 2019-01-07 기간 주문의 주문번호, 고객아이디, 사원번호, 주문일시, 소계, 배송비, 전체금액을 조회하세요.
--      단, 전체금액을 기준으로 내림차순 정렬해서 조회하세요.
        select * from order_header 
			where order_date between '2019-01-01'and '2019-01-08' order by total_due desc;
...
--      단, 사원번호를 기준으로 오름차순, 같은 사원번호는 주문일시를 기준으로 내림차순 정렬해서 조회하세요.
        select * from order_header 
			where order_date between '2019-01-01'and '2019-01-08' order by employee_id, order_date desc;
/**
	그룹함수
**/
/** customer 테이블 사용 **/
