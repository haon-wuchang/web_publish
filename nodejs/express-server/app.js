...
/* GET / POST 방식으로 요청이 들어오는 처리를 여기 자리에 나열한다
 ...
 이면 path : /(root) */
 server.get('/', (req,res)=>{
    res.send('<h1>express 서버에서 전송합니다</h1>'); // 브라우저는 html 로 데이터를 받으니까 안에 내용은 태그를 사용해서 내용을 작성한다
 });   
 /* (req,res) 는 콜백함수의 디폴트 인자인다 
  req : 클라이언트가 보내는 요청정보(클라이언트 -> 서버)
  res : 서버가 클라이언트에게 보내는 응답하는 정보 (서버 -> 클라이언트=브라우저) */
//브라우저는 서버에서 res 결과를 못받으면 계속 로딩중으로 뜸

 /*브라우저가 접속하는 path(경로) : http://localhost:8080/test  
 이면 path : /test 임 */
server.get('/test',(req,res)=>{
    res.send('<h1>퓨퓨퓨퓨 </h1>');
    console.log('/test 전송완료');  // 이거 콘솔은 cmder 에 찍힘용 
});
...
//server.get('/param/:data',(req,res)=>{   
    //console.log('req',req);  이케쳐서 cmder 보면 params { name : 홍길동 } 이케 들어가잇음 그래서 로그찍을때 경로를 저렇게 준거임
